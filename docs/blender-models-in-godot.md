...
A stable armature gives clips consistent targets, supports blending in Godot, and makes later model changes less likely to invalidate animation tracks. Follow [Blender Rigging Best Practices](./blender-rigging-best-practices.md) for skeleton, skinning, and export requirements and [Blender Animation Best Practices](./blender-animation-best-practices.md) for action authoring and validation.

Godot integration depends on stable exported bone names, hierarchy, rest transforms, and action names. Update affected imports, tracks, attachments, and wrapper scenes together when any of those identifiers intentionally change.
