...
func setup(p_side: String, p_definition: UnitDefinition) -> void:
	side = p_side
	...
	phase = randf() * TAU
	name = "%s_%s_%d" % [definition.id, side, get_instance_id()]


func _ready() -> void:
	var key := definition.id
	var model_resource := definition.presentation_scene
	if definition.presentation == "authored_frames":
		_setup_broadsword_2d()
	elif definition.presentation == "model_viewport" and model_resource:
...
func _draw() -> void:
	if not alive:
		return
	var s: float = definition.visual_scale
...
func _bar_w() -> float:
	return 22.0 * definition.visual_scale / 0.13
